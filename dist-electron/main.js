import { ipcMain, shell as shell$1, app, Notification, BrowserWindow } from "electron";
import { fileURLToPath } from "node:url";
import path$4 from "node:path";
import os$9 from "node:os";
import { execSync as execSync$c } from "node:child_process";
...
const execPromise = util$h.promisify(require$$3.exec);
...
function isUefiLinux() {
  return new Promise((resolve) => {
    process.nextTick(() => {
      fs$8.stat("/sys/firmware/efi", (err) => {
        if (!err) {
          return resolve(true);
        } else {
...
const execFileSync$1 = require$$3.execFileSync;
const util$c = util$i;
...
function graphics(callback) {
  ...
  function nvidiaSmi(options) {
    ...
    options = Object.assign({}, options || util$c.execOptsWin);
    if (nvidiaSmiExe) {
...
function fsSize(drive, callback) {
  ...
  let osMounts = [];
  function macOsFsType(fs2) {
    const type = macOsFsTypes.get(fs2);
    if (!type) {
      return getmacOsFsType(fs2);
    }
  ...
  function applyZfsUsage(data, cb) {
    ...
    exec$9("zfs list -H -p -o name,used,avail,mountpoint", { ...util$b.execOptsLinux, timeout: 5e3 }, (error, stdout) => {
      if (error) {
        return cb(data);
      }
      const byMount = /* @__PURE__ */ Object.create(null);
        ...
        if (_darwin$9) {
          ...
          try {
            ...
            execSync$5("mount").toString().split("\n").forEach((line) => {
              const fs2 = line.split(" ")[0];
              const type = line.match(/\(([^),]+)[^)]*\)$/);
              if (fs2 && type) {
                macOsFsTypes.set(fs2, type[1].trim().toLowerCase());
              }
              if (line.startsWith("/")) {
                osMounts[fs2] = line.toLowerCase().indexOf("read-only") === -1;
              }
            });
          } catch {
            util$b.noop();
...
const readFileSync = require$$1.readFileSync;
...
async function getSystemInfo() {
  const [cpu2, mem2, osInfo2, cpuLoad, graphics2, net2] = await Promise.all([
    si.cpu(),
    si.mem(),
    si.osInfo(),
    si.currentLoad(),
  ...
  const gpu = graphics2.controllers[0] || null;
  const primaryNet = net2.find((n) => !n.internal && n.ip4) || net2[0];
  return {
    cpu: {
      name: cpu2.brand || cpu2.manufacturer + " " + cpu2.model || "N/A",
      cores: cpu2.cores,
      usage: Math.round(cpuLoad.currentLoad),
      speed: cpu2.speed || 0
    },
    gpu: {
      name: (gpu == null ? void 0 : gpu.model) || "N/A",
      vendor: (gpu == null ? void 0 : gpu.vendor) || "N/A",
      usage: 0,
      vramTotal: (gpu == null ? void 0 : gpu.vram) ? gpu.vram * 1024 * 1024 : 0,
      vramUsed: 0
    },
    ram: {
      total: mem2.total,
      used: mem2.used,
      free: mem2.free,
      percentage: Math.round(mem2.used / mem2.total * 100)
    },
    network: {
      ip: (primaryNet == null ? void 0 : primaryNet.ip4) || "N/A",
      iface: (primaryNet == null ? void 0 : primaryNet.iface) || "N/A",
      speed: (primaryNet == null ? void 0 : primaryNet.speed) || 0
    },
    uptime: os$9.uptime(),
    system: {
      ...
      arch: osInfo2.arch,
      windowsVersion: `${osInfo2.distro} ${osInfo2.release}`
    }
  };
}
...
function executeCommand(command) {
  try {
    execSync$c(command, { encoding: "utf-8", timeout: 3e4, windowsHide: true });
...
ipcMain.handle("optimize-network", () => {
  ...
  return lastResult.success ? { success: true, message: "Network optimized" } : { success: false, message: "Failed to optimize network" };
});
...
ipcMain.handle("get-system-audit", async () => {
  try {
    const [disk, mem2, cpu2] = await Promise.all([
      si.fsSize(),
      si.mem(),
      si.currentLoad()
    ]);
    ...
    const ramPercent = Math.round(mem2.used / mem2.total * 100);
    if (ramPercent > 85) {
    ...
    const cpuPercent = Math.round(cpu2.currentLoad);
    if (cpuPercent > 85) {
...
ipcMain.handle("enable-ultimate-performance", () => {
  ...
  return fallback.success ? { success: true, message: "High Performance plan enabled" } : { success: false, message: "Failed to change power plan" };
});
ipcMain.handle("disable-unnecessary-services", () => {
  ...
  return count > 0 ? { success: true, message: `${count} service(s) disabled` } : { success: false, message: "No services could be disabled" };
});
...
ipcMain.handle("revert-enable-ultimate-performance", () => {
  ...
  return result.success ? { success: true, message: "Power plan restored" } : { success: false, message: "Failed to restore power plan" };
});
ipcMain.handle("revert-disable-unnecessary-services", () => {
  const services2 = ["SysMain", "DiagTrack"];
  let count = 0;
  for (const svc of services2) {
  ...
  return count > 0 ? { success: true, message: `${count} service(s) restored` } : { success: false, message: "No services could be restored" };
});
...
ipcMain.handle("check-driver-status", () => {
  try {
    const output = execSync$c(
      ...
      { encoding: "utf-8", timeout: 15e3, windowsHide: true }
    );
...
ipcMain.handle("check-network-config", () => {
  try {
    const output = execSync$c("netsh int show interface", { encoding: "utf-8", timeout: 1e4, windowsHide: true });
    const connected = output.includes("Connected");
    return connected ? { success: true, status: "ok", detail: "Rede conectada" } : { success: true, status: "warning", detail: "Sem conexao de rede detectada" };
  } catch {
...
ipcMain.handle("check-system-files", () => {
  try {
    const output = execSync$c(
      ...
      { encoding: "utf-8", timeout: 6e4, windowsHide: true }
    );
...
ipcMain.handle("open-windows-tool", (_event, tool) => {
...
ipcMain.handle("check-for-updates", async () => {
  ...
  try {
    ...
    const latestVersion = ((_a = release.tag_name) == null ? void 0 : _a.replace(/^v/, "")) || "0.0.0";
...
ipcMain.handle("download-update", async (_event, url) => {
  const downloadsDir = path$4.join(app.getPath("downloads"), "ZynVault-Update");
  if (!fs$b.existsSync(downloadsDir)) fs$b.mkdirSync(downloadsDir, { recursive: true });
  const fileName = url.split("/").pop() || "update.exe";
  const filePath = path$4.join(downloadsDir, fileName);
  return new Promise((resolve) => {
    const file = fs$b.createWriteStream(filePath);
    https.get(url, { headers: { "User-Agent": "ZynVault-Updater" } }, (res) => {
      if (res.statusCode === 302 || res.statusCode === 301) {
        httpsGet(res.headers.location).then((redirectUrl) => {
          https.get(redirectUrl, { headers: { "User-Agent": "ZynVault-Updater" } }, (res2) => {
            const totalBytes2 = parseInt(res2.headers["content-length"] || "0", 10);
            let downloaded2 = 0;
            res2.on("data", (chunk) => {
              downloaded2 += chunk.length;
              file.write(chunk);
              if (win) {
                win.webContents.send("update-download-progress", { downloaded: downloaded2, total: totalBytes2, percent: totalBytes2 > 0 ? Math.round(downloaded2 / totalBytes2 * 100) : 0 });
              }
            });
            res2.on("end", () => {
              file.end();
...
ipcMain.handle("set-startup", async (_event, enabled) => {
  try {
    ...
    if (enabled) {
      execSync$c(`reg add "${key}" /v ZynVault /t REG_SZ /d "${exePath}" /f`, { windowsHide: true });
    } else {
      execSync$c(`reg delete "${key}" /v ZynVault /f`, { windowsHide: true });
    }
    return { success: true };
  } catch {
...
ipcMain.handle("get-startup", () => {
  try {
    const key = "HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Run";
    const result = execSync$c(`reg query "${key}" /v ZynVault`, { encoding: "utf-8", windowsHide: true });
...
ipcMain.handle("run-ping", (_event, target) => {
  const sanitized = target.replace(/[^a-zA-Z0-9.\-:]/g, "");
  if (!sanitized || sanitized.length > 253) {
    return { success: false, reachable: false, time: void 0 };
  }
  try {
    const output = execSync$c(`ping -n 1 -w 2000 ${sanitized}`, {
      encoding: "utf-8",
      timeout: 5e3,
      windowsHide: true
    });
    const match = output.match(/time[=<](\d+)ms/i);
    const time2 = match ? `${match[1]}ms` : "OK";
    return { success: true, reachable: true, time: time2 };
  } catch {
    return { success: true, reachable: false, time: void 0 };
  }
});
